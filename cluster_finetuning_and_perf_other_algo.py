...
#print(np.sum(np.square(feats1),axis=1))
#exit(1)

#knn1=np.load('./knn/part3_test_k15.npz')
...
label_predict=np.loadtxt('C:/Users/isetsk/Downloads/cdp-master/cdp-master/experiments/affinity_single/output/k15_vote_accept0_th0.7/sz600_step0.05/meta.txt')
...
#permute the labels
#r_perm=np.random.permutation(len(label_true))
#label_true=label_true[r_perm]
#label_predict=label_predict[r_perm]
#label_predict2=label_predict2[r_perm]
#feats1=feats1[r_perm,:]
#knn1=knn1[r_perm,:]
...
label_no_process[count<=150]=1#150
for j in range(0,3):
	print('iter='+str(j))
	print(next_label)
	for i in range(0,next_label.astype(int)):	
		...
		#kmeans1=KMeans(n_clusters=1,random_state=0).fit(features1)
		#center=np.sum(features1,axis=0)/np.size(features1,0)
		#error1=(np.mean(np.sum(np.square(features1-center),axis=1)))
		#kmeans2=KMeans(n_clusters=2,random_state=0,n_init=1).fit(features1)#n_init=10
		#error2=kmeans2.inertia_/np.size(features1,0)
		#labels=kmeans2.labels_
		
		#bandwidth = estimate_bandwidth(features1, quantile=1, n_samples=None)#quantile=0.9 n_samples=500
		#bandwidth=1.1
		...
		#'''
		#Z = ward(pdist(features1,'euclidean'))
		Z = linkage(features1, 'ward',optimal_ordering=False)
		temp_z=Z[::-1,2]
		#temp_z=np.square(temp_z)
		##print(temp_z)
		n_clusters=1
		temp_z_ind=np.where((temp_z[0:13]-temp_z[1:14])>1.1)#1.1)
		##print(temp_z_ind)
		if len(temp_z_ind[0])>0:
			n_clusters=(temp_z_ind[0][-1]+2)
		#print(n_clusters)
		##print(temp_z[1]-temp_z[0])
		##print(n_clusters)
		#n_clusters=2
		labels=fcluster(Z,n_clusters, criterion='maxclust')#criterion='distance')
		##labels=fcluster(Z,t=thres,criterion='distance')
		labels=labels-1
		#print(labels)
		#'''
		
		#clusters=AgglomerativeClustering(n_clusters=2, affinity='euclidean', linkage='average')
		#clusters.fit_predict(features1)
		#labels=clusters.labels_
		#clustering = DBSCAN(eps=0.75, min_samples=2).fit(features1)
		#labels=clustering.labels_
		#clust = OPTICS(min_samples=50, xi=.05, min_cluster_size=.05)
		#clust.fit(features1)
		#labels=clust.labels_
		#brc = Birch(branching_factor=50, n_clusters=2,  compute_labels=True)
		#brc.fit(features1)
		#labels=brc.labels_
		#print(labels)
		#print(labels)
		
		#a,labels=np.unique(label_true[sel],return_inverse=True)
		#n_clusters=np.max(labels)+1
		#print(labels)
		#cluster_center=np.zeros((2,np.size(features1,1)))
		#cluster_center[0,:]=np.sum(features1[labels==0,:],axis=0)/np.sum(labels==0)
		#cluster_center[1,:]=np.sum(features1[labels==1,:],axis=0)/np.sum(labels==1)
		#error2=(np.mean(np.sum(np.square(features1-cluster_center[labels,:]),axis=1)))
	
		
		
		
		#print(i)
		#Z = ward(pdist(features1,'cosine'))
		#labels=fcluster(Z, t=0.5, criterion='distance')
		#dendrogram = sch.dendrogram(sch.linkage(features1, method='ward'))
		#for key in sorted(dendrogram.keys()):
		#	print( key)#, dict[key]
		#cluster = AgglomerativeClustering( affinity='euclidean', linkage='ward',t=0.4)#,distance_threshold=0.4)
		#print(dir(cluster))
		#cluster.fit_predict(features1)
...
print(np.max(label_predict2))
print(np.max(label_predict))
